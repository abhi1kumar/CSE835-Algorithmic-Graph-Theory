...
#define USAGE \
"dp [infile [outfile]] [-gmpsu]"

#define HELPTEXT \
"Remove non distance preserving graphs from a file of graphs.\n\n\
    -g    :  use graph6 output\n\
...
int main(int argc, char *argv[])
{
    ...
    if (gswitch + mswitch + pswitch + sswitch + uswitch + zswitch > 1)
        gt_abort(">E dp: -gmpsuz are incompatible\n");
    if (badargs)
    {
    ...
    for (t = CPUTIME; ; free(g))
    {
        ...
        if ((g = readg(infile, NULL, 0, &m, &n)) == NULL)
            break;
        if (!isconnected(g, m, n) || distance_preserving(g, m, n))
            continue;
